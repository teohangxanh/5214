...
def filter_date(df, given_id, start_date, end_date) -> None:
    '''Filter the entire dataset to return only rows of the restaurant id'''
    rest = df[df['restaurant_id'] == given_id].reset_index(drop=True)
    # Convert date column to datetime
    rest['date'] = rest.to_datetime(df['date'])
    # Filter the reviews according to date
    filtered_date = rest[(rest['date'] >= start_date) & (rest['date'] <= end_date)]
    filtered_date.to_csv(f'Filtered of restaurant {given_id} from {start_date} to {end_date}.csv', index=False)